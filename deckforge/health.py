"""DeckForge health: per-deck outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "dk_inputs"
OUTPUTS_DIR = DATA_DIR / "dk_outputs"
FILE_LOG    = DATA_DIR / "dk_deck_log.json"
DELIVERY    = DATA_DIR / "dk_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("DK_LOG_MAX", "300"))
VALID = {'success', 'build_failed', 'no_slides', 'spec_invalid'}
...
def record_deck(slug, outcome, detail="", **extra):
    if not slug: return
...
def recent_decks(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def deck_outcome_summary():
    log = _load(FILE_LOG, [])
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "dk_inputs": n_in, "dk_outputs": n_out,
