"""StoryForge health: per-prompt outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "sf_inputs"
OUTPUTS_DIR = DATA_DIR / "sf_outputs"
FILE_LOG    = DATA_DIR / "sf_prompt_log.json"
DELIVERY    = DATA_DIR / "sf_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("SF_LOG_MAX", "300"))
VALID = {'success', 'build_failed', 'no_active_project'}
...
def record_prompt(slug, outcome, detail="", **extra):
    if not slug: return
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "sf_inputs": n_in, "sf_outputs": n_out,
