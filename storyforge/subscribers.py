"""StoryForge subscribers. Daily prompts ($19/mo) · Weekly tracker ($49/mo) · Story bible ($197)."""
...
SUBS = DATA_DIR / "sf_subscribers.json"
LOG  = DATA_DIR / "sf_subscription_log.json"

PLANS = {
    "daily_19": {
        "price_mo": 19,
        "one_time": 0,
        "label": "Daily prompts ($19/mo)"
    },
    "weekly_49": {
        "price_mo": 49,
        "one_time": 0,
        "label": "Weekly tracker ($49/mo)"
    },
    "bible_197": {
        "price_mo": 0,
        "one_time": 197,
        "label": "Story bible ($197)"
    }
}
...
def activate(email):
    ...
    if not isinstance(subs, list): return {"error": "sf_subscribers.json wrong shape"}
    for s in subs:
