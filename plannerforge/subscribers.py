"""PlannerForge subscribers. Per-planner ($19) · Monthly ($49/mo) · Annual planner ($197)."""
...
SUBS = DATA_DIR / "pl_subscribers.json"
LOG  = DATA_DIR / "pl_subscription_log.json"

PLANS = {
    "per_planner_19": {
        "price_mo": 0,
        "one_time": 19,
        "label": "Per-planner ($19)"
    },
    "monthly_49": {
        "price_mo": 49,
        "one_time": 0,
...
def activate(email):
    ...
    if not isinstance(subs, list): return {"error": "pl_subscribers.json wrong shape"}
    for s in subs:
