"""PlannerForge health: per-planner outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "pl_inputs"
OUTPUTS_DIR = DATA_DIR / "pl_outputs"
FILE_LOG    = DATA_DIR / "pl_planner_log.json"
DELIVERY    = DATA_DIR / "pl_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("PL_LOG_MAX", "300"))
VALID = {'success', 'bad_dates', 'build_failed', 'spec_invalid'}
...
def record_planner(slug, outcome, detail="", **extra):
    if not slug: return
...
def recent_planners(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def planner_outcome_summary():
    log = _load(FILE_LOG, [])
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "pl_inputs": n_in, "pl_outputs": n_out,
