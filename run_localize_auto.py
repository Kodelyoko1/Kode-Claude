#!/usr/bin/env python3
"""Localize autonomous loop — translate input + auto-source, deliver to subs, pitch leads."""
...
def cycle():
    console.print(Panel(
        ...
        title="[bold blue]Wholesale Omniverse — Localize[/bold blue]",
        border_style="blue"))
    r = run_full_cycle()
    console.print(f"  [cyan]Translations produced:[/cyan] {r.get('translations_produced', 0)}")
...
def main():
    ...
    if not paywall_prompt("localize"):
        return
    while True:
        cycle()
        if args.interval <= 0:
