...
from gutenberg_voice.health import (probe_inputs, script_outcome_summary, stuck_mail_failed)
from gutenberg_voice.subscribers import listing as sub_listing

DATA_DIR = Path(__file__).parent.parent / "data"
LEADS = DATA_DIR / "gv_leads.json"
...
def check_inputs():
    p = probe_inputs()
    if not p.get("ok"):
        return Check("Inputs", "P0", "fail", "gv_inputs/ empty",
                     "Drop manifest at data/gv_inputs/<slug>.json")
    age = p.get("newest_age_days")
    age_s = f" newest {age}d old" if age is not None else ""
    detail = f"gv_inputs={p['gv_inputs']}  gv_outputs={p['gv_outputs']}{age_s}"
...
def check_outcomes():
    ...
    if s["total"] == 0: return Check("Outcomes", "info", "info", "(no scripts logged)")
...
def check_leads():
    ...
    if not isinstance(leads, list) or not leads:
        return Check("Lead pipeline", "info", "info", "0 leads — populate gv_leads.json")
...
def main():
    print("GutenbergVoice preflight\n")
