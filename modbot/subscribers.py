"""ModBot subscribers. Per-account ($97/mo) · Team 5-account ($297/mo) · One-time audit ($497)."""
...
SUBS = DATA_DIR / "cm_subscribers.json"
LOG  = DATA_DIR / "cm_subscription_log.json"

PLANS = {
    "monthly_97": {
        "price_mo": 97,
        "one_time": 0,
        "label": "Per-account ($97/mo)"
    },
    "team_297": {
        "price_mo": 297,
        "one_time": 0,
        "label": "Team 5-account ($297/mo)"
    },
    "audit_497": {
        "price_mo": 0,
        "one_time": 497,
        "label": "One-time audit ($497)"
    }
}
...
def activate(email):
    ...
    if not isinstance(subs, list): return {"error": "cm_subscribers.json wrong shape"}
    for s in subs:
