"""ModBot health: per-batch outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "cm_inputs"
OUTPUTS_DIR = DATA_DIR / "cm_outputs"
FILE_LOG    = DATA_DIR / "cm_batch_log.json"
DELIVERY    = DATA_DIR / "cm_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("CM_LOG_MAX", "300"))
VALID = {'success', 'no_comments', 'build_failed', 'spec_invalid'}
...
def record_batch(slug, outcome, detail="", **extra):
    if not slug: return
...
def recent_batchs(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def batch_outcome_summary():
    log = _load(FILE_LOG, [])
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "cm_inputs": n_in, "cm_outputs": n_out,
