#!/usr/bin/env python3
"""DomainScout autonomous loop — generate + check domain lists."""
...
def cycle():
    console.print(Panel(
        ...
        title="[bold blue]Wholesale Omniverse — DomainScout[/bold blue]",
        border_style="blue"))
    r = run_full_cycle()
    console.print(f"  [cyan]Reports built:[/cyan]   {r.get('reports_produced', 0)}")
...
def main():
    ...
    if not paywall_prompt("domainscout"):
        return
    while True:
        cycle()
        if args.interval <= 0:
