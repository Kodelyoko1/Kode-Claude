"""BentoForge subscribers. Plans: per_page_19 ($19 one-time),
hosting_9 ($9/mo), white_label_49 ($49 one-time terminal)."""
...
SUBS = DATA_DIR / "bf_subscribers.json"
LOG  = DATA_DIR / "bf_subscription_log.json"

PLANS = {
    "per_page_19":     {"price_mo": 0, "one_time": 19, "label": "Single page ($19)"},
    "hosting_9":       {"price_mo": 9, "one_time": 0,  "label": "Monthly hosting ($9/mo)"},
    "white_label_49":  {"price_mo": 0, "one_time": 49, "label": "White-label ($49)"},
}
...
def activate(email):
    ...
    if not isinstance(subs, list): return {"error": "bf_subscribers.json wrong shape"}
    for s in subs:
