"""BentoForge health: per-page outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "bf_inputs"
OUTPUTS_DIR = DATA_DIR / "bf_outputs"
FILE_LOG    = DATA_DIR / "bf_page_log.json"
DELIVERY    = DATA_DIR / "bf_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("BF_LOG_MAX", "300"))
VALID_OUTCOMES = {"success", "spec_invalid", "no_links", "build_failed"}
...
def record_page(slug, outcome, theme="", link_count=0, detail=""):
    if not slug: return
    _append(FILE_LOG, {"ts": _now(), "slug": slug, "outcome": outcome,
                       "theme": theme, "link_count": int(link_count), "detail": detail})
...
def recent_pages(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def page_outcome_summary():
    log = _load(FILE_LOG, [])
    if not isinstance(log, list) or not log:
        return {"total": 0, **{oc: 0 for oc in VALID_OUTCOMES}, "by_theme": {}}
    counts = {oc: 0 for oc in VALID_OUTCOMES}
    by_theme = {}
    for r in log:
        oc = r.get("outcome", "")
        if oc in counts: counts[oc] += 1
        if oc == "success":
            t = r.get("theme", "?")
            by_theme[t] = by_theme.get(t, 0) + 1
    return {"total": len(log), **counts, "by_theme": by_theme}
...
def probe_inputs():
    ...
    if OUTPUTS_DIR.exists():
        n_out = sum(1 for d in OUTPUTS_DIR.iterdir() if d.is_dir())
    newest_age = None
    if INPUTS_DIR.exists():
        files = list(INPUTS_DIR.glob("*.json"))
        if files:
            m = max(f.stat().st_mtime for f in files)
            newest_age = (datetime.now() - datetime.fromtimestamp(m)).days
    return {"ok": n_in > 0, "bf_inputs": n_in, "bf_outputs": n_out,
