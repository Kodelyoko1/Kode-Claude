...
from bentoforge.health import (probe_inputs, page_outcome_summary, stuck_mail_failed)
from bentoforge.subscribers import listing as sub_listing

DATA_DIR = Path(__file__).parent.parent / "data"
LEADS = DATA_DIR / "bf_leads.json"
OUTPUTS_DIR = DATA_DIR / "bf_outputs"
...
def check_inputs():
    p = probe_inputs()
    if not p.get("ok"):
        return Check("Inputs", "P0", "fail", "bf_inputs/ empty",
                     "Drop a manifest at data/bf_inputs/<slug>.json")
    age = p.get("newest_age_days")
    age_s = f" newest {age}d old" if age is not None else ""
    detail = f"bf_inputs={p['bf_inputs']}  bf_outputs={p['bf_outputs']}{age_s}"
    if age is not None and age > 30:
        return Check("Inputs", "P1", "warn", detail,
                     "Newest input >30d old — owner stopped feeding")
    return Check("Inputs", "info", "info", detail)


def check_outcomes():
    s = page_outcome_summary()
    if s["total"] == 0: return Check("Outcomes", "info", "info", "(no pages logged)")
    by_theme = " ".join(f"{k}={v}" for k, v in s["by_theme"].items())
    detail = (f"log={s['total']}  success={s['success']}  spec_invalid={s['spec_invalid']}  "
              f"no_links={s['no_links']}  build_failed={s['build_failed']}"
              + (f" · themes: {by_theme}" if by_theme else ""))
    fail = s["spec_invalid"] + s["no_links"] + s["build_failed"]
...
def check_stuck_mail():
    ...
    return Check("Stuck mail_failed", "P1", "warn",
                 f"{len(stuck)} recipient(s) stuck: {sample}",
                 "Pages built but mailer keeps rejecting")
...
def check_leads():
    ...
    if not isinstance(leads, list) or not leads:
        return Check("Lead pipeline", "info", "info", "0 leads — populate bf_leads.json")
...
def run_diagnostics():
    checks = [check_smtp(), check_inputs(), check_outcomes(), check_stuck_mail(),
              check_subscribers(), check_leads()]
