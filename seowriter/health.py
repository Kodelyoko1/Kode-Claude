"""SEOWriter health: per-article outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "sw_inputs"
OUTPUTS_DIR = DATA_DIR / "sw_outputs"
FILE_LOG    = DATA_DIR / "sw_article_log.json"
DELIVERY    = DATA_DIR / "sw_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("SW_LOG_MAX", "300"))
VALID = {'success', 'no_keyword', 'build_failed', 'spec_invalid'}
...
def record_article(slug, outcome, detail="", **extra):
    if not slug: return
...
def recent_articles(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def article_outcome_summary():
    log = _load(FILE_LOG, [])
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "sw_inputs": n_in, "sw_outputs": n_out,
