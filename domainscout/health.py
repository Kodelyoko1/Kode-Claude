"""DomainScout health: per-list outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "dm_inputs"
OUTPUTS_DIR = DATA_DIR / "dm_outputs"
FILE_LOG    = DATA_DIR / "dm_list_log.json"
DELIVERY    = DATA_DIR / "dm_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("DM_LOG_MAX", "300"))
VALID = {'success', 'build_failed', 'spec_invalid', 'no_domains'}
...
def record_list(slug, outcome, detail="", **extra):
    if not slug: return
...
def recent_lists(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def list_outcome_summary():
    log = _load(FILE_LOG, [])
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "dm_inputs": n_in, "dm_outputs": n_out,
