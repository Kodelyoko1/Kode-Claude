...
from domainscout.health import (probe_inputs, list_outcome_summary, stuck_mail_failed)
from domainscout.subscribers import listing as sub_listing

DATA_DIR = Path(__file__).parent.parent / "data"
LEADS = DATA_DIR / "dm_leads.json"
...
def check_inputs():
    p = probe_inputs()
    if not p.get("ok"):
        return Check("Inputs", "P0", "fail", "dm_inputs/ empty",
                     "Drop manifest at data/dm_inputs/<slug>.json")
    age = p.get("newest_age_days")
    age_s = f" newest {age}d old" if age is not None else ""
    detail = f"dm_inputs={p['dm_inputs']}  dm_outputs={p['dm_outputs']}{age_s}"
...
def check_outcomes():
    ...
    if s["total"] == 0: return Check("Outcomes", "info", "info", "(no lists logged)")
...
def check_leads():
    ...
    if not isinstance(leads, list) or not leads:
        return Check("Lead pipeline", "info", "info", "0 leads — populate dm_leads.json")
