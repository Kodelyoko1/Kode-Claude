...
from templateforge.health import (probe_inputs, template_outcome_summary, stuck_mail_failed)
from templateforge.subscribers import listing as sub_listing

DATA_DIR = Path(__file__).parent.parent / "data"
LEADS = DATA_DIR / "td_leads.json"
...
def check_inputs():
    p = probe_inputs()
    if not p.get("ok"):
        return Check("Inputs", "P0", "fail", "td_inputs/ empty",
                     "Drop manifest at data/td_inputs/<slug>.json")
    age = p.get("newest_age_days")
    age_s = f" newest {age}d old" if age is not None else ""
    detail = f"td_inputs={p['td_inputs']}  td_outputs={p['td_outputs']}{age_s}"
...
def check_leads():
    ...
    if not isinstance(leads, list) or not leads:
        return Check("Lead pipeline", "info", "info", "0 leads — populate td_leads.json")
