"""TemplateForge subscribers. Per-template ($19) · 5-template pack ($69) · Monthly templates ($99/mo)."""
...
SUBS = DATA_DIR / "td_subscribers.json"
LOG  = DATA_DIR / "td_subscription_log.json"
...
PLANS = {
    ...
    "pack_5_69": {
        "price_mo": 0,
        "one_time": 69,
        "label": "5-template pack ($69)"
    },
    "monthly_99": {
        "price_mo": 99,
        "one_time": 0,
        "label": "Monthly templates ($99/mo)"
    }
}
...
def activate(email):
    ...
    if not isinstance(subs, list): return {"error": "td_subscribers.json wrong shape"}
    for s in subs:
