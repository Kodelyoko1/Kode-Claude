#!/usr/bin/env python3
"""DeckForge autonomous loop — render browser-native pitch decks."""
...
def cycle():
    console.print(Panel(
        ...
        title="[bold blue]Wholesale Omniverse — DeckForge[/bold blue]",
        border_style="blue"))
    r = run_full_cycle()
    console.print(f"  [cyan]Decks built:[/cyan]    {r.get('decks_produced', 0)}")
...
def main():
    ...
    if not paywall_prompt("deckforge"):
        return
    while True:
        cycle()
        if args.interval <= 0:
