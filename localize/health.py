"""Localize health: per-translation outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "lz_inputs"
OUTPUTS_DIR = DATA_DIR / "lz_outputs"
FILE_LOG    = DATA_DIR / "lz_translation_log.json"
DELIVERY    = DATA_DIR / "lz_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("LZ_LOG_MAX", "300"))
VALID = {'success', 'mail_failed', 'spec_invalid', 'no_source_text', 'translation_failed'}
...
def record_translation(slug, outcome, detail="", **extra):
    if not slug: return
...
def recent_translations(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def translation_outcome_summary():
    log = _load(FILE_LOG, [])
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "lz_inputs": n_in, "lz_outputs": n_out,
