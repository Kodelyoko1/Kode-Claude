"""PodCleaner subscribers. Per-episode ($9) · Monthly 10-ep ($49/mo) · 30-episode bulk ($199)."""
...
SUBS = DATA_DIR / "pd_subscribers.json"
LOG  = DATA_DIR / "pd_subscription_log.json"
...
PLANS = {
    ...
    "monthly_49": {
        "price_mo": 49,
        "one_time": 0,
        "label": "Monthly 10-ep ($49/mo)"
    },
    "bulk_30_199": {
        "price_mo": 0,
        "one_time": 199,
        "label": "30-episode bulk ($199)"
    }
}
...
def activate(email):
    ...
    if not isinstance(subs, list): return {"error": "pd_subscribers.json wrong shape"}
    for s in subs:
