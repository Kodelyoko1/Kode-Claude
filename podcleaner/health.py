"""PodCleaner health: per-episode outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "pd_inputs"
OUTPUTS_DIR = DATA_DIR / "pd_outputs"
FILE_LOG    = DATA_DIR / "pd_episode_log.json"
DELIVERY    = DATA_DIR / "pd_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("PD_LOG_MAX", "300"))
VALID = {'success', 'ffmpeg_missing', 'unsupported_ext', 'ffmpeg_failed'}
...
def record_episode(slug, outcome, detail="", **extra):
    if not slug: return
...
def recent_episodes(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def episode_outcome_summary():
    log = _load(FILE_LOG, [])
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "pd_inputs": n_in, "pd_outputs": n_out,
