"""ChatConfig subscribers. Plans: setup_99 ($99 one-time), monitoring_49 ($49/mo), multi_bot_297 ($297 one-time)."""
...
SUBS = DATA_DIR / "cc_subscribers.json"
LOG  = DATA_DIR / "cc_subscription_log.json"

PLANS = {
    "setup_99":      {"price_mo": 0,  "one_time": 99,  "label": "One-time setup ($99)"},
    "monitoring_49": {"price_mo": 49, "one_time": 0,   "label": "Monthly monitoring ($49/mo)"},
    "multi_bot_297": {"price_mo": 0,  "one_time": 297, "label": "Multi-bot pack ($297)"},
}
...
def activate(email):
    ...
    if not isinstance(subs, list): return {"error": "cc_subscribers.json wrong shape"}
    for s in subs:
