...
from chatconfig.health import (probe_inputs, bot_outcome_summary, stuck_mail_failed)
from chatconfig.subscribers import listing as sub_listing

DATA_DIR = Path(__file__).parent.parent / "data"
LEADS = DATA_DIR / "cc_leads.json"
...
def check_smtp():
    ...
    if not (user and pwd): return Check("SMTP creds", "P0", "fail", "SMTP_USER/SMTP_PASS not set", "Required for delivery")
    try:
        with smtplib.SMTP(os.environ.get("SMTP_HOST", "smtp.gmail.com"), int(os.environ.get("SMTP_PORT", "587")), timeout=10) as s:
...
def check_inputs():
    p = probe_inputs()
    if not p.get("ok"): return Check("Inputs", "P0", "fail", "cc_inputs/ empty", "Drop manifest at data/cc_inputs/<slug>.json")
    age = p.get("newest_age_days")
    age_s = f" newest {age}d old" if age is not None else ""
    detail = f"cc_inputs={p['cc_inputs']}  cc_outputs={p['cc_outputs']}{age_s}"
    if age is not None and age > 30:
        return Check("Inputs", "P1", "warn", detail, "Newest input >30d old")
    return Check("Inputs", "info", "info", detail)


def check_outcomes():
    s = bot_outcome_summary()
    if s["total"] == 0: return Check("Outcomes", "info", "info", "(no bots logged)")
    detail = f"log={s['total']}  success={s['success']}  spec_invalid={s['spec_invalid']}  no_faqs={s['no_faqs']}  build_failed={s['build_failed']}"
    fail = s['spec_invalid'] + s['no_faqs'] + s['build_failed']
...
def check_stuck_mail():
    stuck = stuck_mail_failed(3)
    if not stuck: return Check("Stuck mail_failed", "info", "info", "(none)")
    return Check("Stuck mail_failed", "P1", "warn",
                 f"{len(stuck)} stuck: " + ", ".join(f"{s['email']}({s['attempts']}×)" for s in stuck[:4]),
                 "Bots built but mailer keeps rejecting")
...
def check_subscribers():
    ...
    return Check("Subscribers", "info", "info",
                 f"active={out['active']}  MRR≈${out['mrr']:.0f}/mo  one-time=${out['one_time_collected']}  · {by_plan}")


def check_leads():
    leads = _load(LEADS, [])
    if not isinstance(leads, list) or not leads:
        return Check("Lead pipeline", "info", "info", "0 leads — populate cc_leads.json")
    teased = sum(1 for l in leads if l.get("trial_sent"))
    return Check("Lead pipeline", "info", "info", f"{len(leads)} lead(s) · trial_sent={teased}")


def run_diagnostics():
    checks = [check_smtp(), check_inputs(), check_outcomes(), check_stuck_mail(), check_subscribers(), check_leads()]
