...
Rule-based Inner Circle Trader (ICT) prediction agent for Gold (GC) and WTI Crude Oil (CL), formatted for MetaTrader 5. Pulls free intraday OHLC candles (Yahoo Finance chart JSON, no key) for the 15M bias frame and a 5M/1M precision frame, then runs the ICT Decision & Validation Matrix in pure price-action code (never LLM-invented levels): Step 1 gates on an active killzone (London 02:00–05:00 and NY AM 07:00–10:00 **New York local time**, resolved through `America/New_York` so the UTC equivalents shift correctly with US daylight saving — 06:00–09:00 / 11:00–14:00 UTC in summer, 07:00–10:00 / 12:00–15:00 in winter; outside both is an automatic NO TRADE), and on the market actually being open (spot gold/FX closes Fri 21:00 UTC, reopens Sun 22:00 UTC); Step 3 maps unswept buy-/sell-side liquidity (DOL) via swing-point fractals on the 15M frame; Step 4 detects a liquidity sweep + strong-bodied displacement candle confirming a Market Structure Shift (MSS) on the precision frame, then a 3-candle Fair Value Gap (FVG) in the displacement leg (entry = FVG midpoint / CE); Step 5 requires ≥1:2 reward:risk against the opposing liquidity pool or the setup is downgraded to NO TRADE. Optional `ANTHROPIC_API_KEY` (Claude `claude-sonnet-4-6`) only rephrases the 2-sentence execution summary from the already-computed numbers — it never sets a price. Output matches the fixed `AUTONOMOUS ICT PREDICTION AGENT REPORT` template. Every LONG/SHORT signal is also turned into an MT5 order plan (`ict_predictor/mt5_execution.py`, optional `MetaTrader5` package, Windows-only/Wine) — **dry-run by default** (order priced, sized off account risk %, and logged as "simulated," nothing sent to a broker); set `IP_MT5_LIVE=1` plus `MT5_LOGIN`/`MT5_PASSWORD`/`MT5_SERVER` in `.env` and a running MT5 terminal to actually place orders. **Demo/test accounts only:** even when armed, `mt5_execution.py` queries `account_info().trade_mode` and refuses to submit unless the account reports demo or contest — the check fails closed, so an account it can't positively identify as demo is treated as real and blocked (status `real_account_blocked`); trading real money requires deliberately setting `IP_MT5_ALLOW_REAL=1`. Not financial advice; substantial risk of loss.  
**Run:** `python3 run_ict_predictor_auto.py` (full cycle) · `--asset GC|CL` (single scan, includes MT5 order preview) · `--status` (recent predictions) · `--backtest [--bars N]` (walk-forward historical replay with edge stats + rejection funnel) · `--sweep` (parameter sensitivity grid with in/out-of-sample split) · `--forward` (scores the agent's OWN logged predictions against what price actually did — out-of-sample by construction, so unlike the backtest it cannot be tuned after the fact; dedupes the same setup re-emitted each 15-min cycle by its MSS timestamp, skips Yahoo-sourced levels as the wrong instrument, and persists results to `data/ip_forward_results.json` so the rolling prediction log can't rotate the evidence away) · `--doctor` (preflight: MT5 package/connection/demo-guard/symbol resolution/price feed/killzone — auto-discovers the broker's real gold+oil symbol names when the configured ones don't exist)  
**Env:** `IP_ASSETS` (default `GC,CL`), `IP_PRICE_SOURCE` (`auto`|`mt5`|`yahoo`, default `auto`), `IP_LTF_INTERVAL` (default `5m`), `IP_MIN_RR` (default `2.0`), `IP_SWEEP_LOOKBACK` (default `12`), `IP_DISPLACEMENT_MULT` (default `1.3` — the MSS filter, tightest gate in the matrix), `IP_MAX_STALE_SEC` (default `1200`), `IP_MT5_MAX_BAR_AGE_SEC` (default: 2× the bar interval, floor 10 min — how far behind the live tick MT5's own history may be before the series is refused, since a freshly selected symbol returns bars the terminal hasn't finished downloading), `IP_MT5_MAX_ORDERS_PER_SYMBOL` (default `1`), `IP_KILLZONE_TZ` (`ny` default, `utc` to pin to the old fixed hours), `IP_MT5_SPREAD_ADJUST` (default `1` — raises BUY entries by the live spread, since MT5 rates are bid-side but a BUY_LIMIT triggers on the ask), `IP_OWNER_EMAIL`, `IP_MT5_LIVE` (default `0`), `IP_MT5_ALLOW_REAL` (default `0` — demo/contest accounts only), `IP_MT5_SYMBOL_GC`/`IP_MT5_SYMBOL_CL` (broker-specific symbol names), `IP_MT5_RISK_PCT` (default `0.5`), `IP_MT5_LOT_SIZE` (fallback), `MT5_LOGIN`/`MT5_PASSWORD`/`MT5_SERVER`/`MT5_PATH`. Backtest/sweep cost model (all in price units — $/oz gold, $/bbl crude): `IP_BT_SPREAD` (default `0.30`, charged on entry), `IP_BT_COMMISSION` (default `0.0`, round-turn — divide the broker's per-lot figure by the contract size), `IP_BT_SLIPPAGE` (default `0.10`, charged on stop exits only, since a limit entry slips by not filling rather than by filling worse), `IP_BT_SWAP` (default `0.15` per night held). Reports show gross, cost drag, and net expectancy side by side, and the sweep ranks settings on NET.  
**Data:** `data/ip_candles/` (cached price feed), `data/ip_predictions.json` (rolling log), `data/ip_forward_results.json` (settled forward-test outcomes, never trimmed), `data/ip_reports/YYYY-MM-DD.md` (digest)  
**Scheduling:** `setup_ict_schedule.ps1` — registers a Windows scheduled task (Mon–Fri 07:00–10:00 local, repeating every 15 min — since the NY AM killzone *is* 07:00–10:00 New York time and Task Scheduler triggers are local, this tracks the window exactly in both seasons with no DST arithmetic; `-Once` for a single daily run, `-Remove` to unregister). Runs **only while logged on** because the MetaTrader5 API attaches to a running terminal and cannot start one — set MT5 to launch with Windows. Logs per run to `data/ip_cron_logs/ict_YYYYMMDD.log`.  
**VPS bootstrap:** `setup_mt5_vps.ps1` — run in PowerShell on a fresh Windows VPS (MetaTrader5's package is Windows-only) to clone this branch, install requirements, and scaffold a local `.env`; see script header for the full walkthrough.
...
### AI Asset Manager Agent (`asset_manager/`) — $147/mo signal + paper-trading feed, $397/mo live execution
Modular, self-contained portfolio monitoring + (paper or live) trade execution agent for crypto, equities, or a custom asset universe — architecturally distinct from the other agents (own `config/connectors/strategies/risk/execution` layout, per-module `BaseConnector`/`BaseStrategy`/`BaseExecutionEngine` abstractions, Pydantic-validated `Signal`/`Order`/`PortfolioState` schemas) so any layer can be swapped without touching the rest. **Paper trading is on by default** — `execution.get_execution_engine()` only ever returns a live engine when `AM_PAPER_TRADING=0` is explicitly set *and* the selected connector reports `is_paper_only = False`, so no single flag arms live trading by accident. Default `AM_CONNECTOR=paper` needs zero API keys (deterministic synthetic per-symbol price walk); swap in `ccxt` (crypto exchanges) or `yfinance` + Alpaca (equities) for real data/execution. Three strategies ship built-in: `rebalance` (default — target-weight drift correction against `config/target_allocations.yaml`), `sma_crossover` (golden/death cross), `momentum` (top-N trailing-return ranking). Every signal passes through `risk/manager.py` — the one choke point enforcing max allocation % per asset, max daily loss (halts new BUYs, still allows risk-reducing SELLs), max slippage tolerance, an orders-per-cycle circuit breaker, and a dust-order floor — before it can become an Order; the risk manager trims oversized orders down to the limit rather than only reject/approve. State (orders, signals with their risk verdict, portfolio snapshots) persists to SQLite at `data/asset_manager.db`. Ships `pytest` unit tests for every strategy and every risk-guardrail branch (`asset_manager/tests/`).  
**Run:** `python3 run_asset_manager_auto.py` (one cycle) · `--loop` (continuous, APScheduler-driven) · `--status` (portfolio snapshot) · `--history` (recent orders) · `--live` (arm live trading for this run)  
**Env:** `AM_PAPER_TRADING` (default `1`), `AM_CONNECTOR` (`paper`|`ccxt`|`yfinance`, default `paper`), `AM_STRATEGY` (`rebalance`|`sma_crossover`|`momentum`), `AM_SYMBOLS`, `AM_EXCHANGE_ID`/`AM_EXCHANGE_API_KEY`/`AM_EXCHANGE_API_SECRET` (ccxt), `AM_ALPACA_API_KEY`/`AM_ALPACA_API_SECRET` (equities live execution), `AM_MAX_ALLOCATION_PCT`/`AM_MAX_DAILY_LOSS_PCT`/`AM_MAX_SLIPPAGE_PCT` (fallback risk defaults — tune `asset_manager/config/risk_limits.yaml` instead), `AM_CYCLE_INTERVAL_SECONDS`, `AM_DB_PATH`. Full list in `.env.example`.  
**Data:** `data/asset_manager.db` (orders/signals/portfolio-snapshot history), `data/am_logs/agent.log`, `data/am_paper_prices.json` (paper connector's synthetic price state)  
**Docs:** `asset_manager/README.md` — architecture, safety mechanics, config reference, going-live checklist.
