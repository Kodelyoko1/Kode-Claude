...
**Env:** `IP_ASSETS` (default `GC,CL`), `IP_PRICE_SOURCE` (`auto`|`mt5`|`yahoo`, default `auto`), `IP_LTF_INTERVAL` (default `5m`), `IP_MIN_RR` (default `2.0`), `IP_SWEEP_LOOKBACK`, `IP_OWNER_EMAIL`, `IP_MT5_LIVE` (default `0`), `IP_MT5_ALLOW_REAL` (default `0` — demo/contest accounts only), `IP_MT5_SYMBOL_GC`/`IP_MT5_SYMBOL_CL` (broker-specific symbol names), `IP_MT5_RISK_PCT` (default `0.5`), `IP_MT5_LOT_SIZE` (fallback), `MT5_LOGIN`/`MT5_PASSWORD`/`MT5_SERVER`/`MT5_PATH`  
