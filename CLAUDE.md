...
**Scheduling:** `setup_ict_schedule.ps1` — registers a Windows scheduled task (Mon–Fri 09:00 local, repeating every 15 min for 3 h to cover the NY AM killzone; `-Once` for a single daily run, `-Remove` to unregister). Runs **only while logged on** because the MetaTrader5 API attaches to a running terminal and cannot start one — set MT5 to launch with Windows. Logs per run to `data/ip_cron_logs/ict_YYYYMMDD.log`.  
