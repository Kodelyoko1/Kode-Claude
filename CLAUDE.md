...
**Run:** `python3 run_ict_predictor_auto.py` (full cycle) · `--asset GC|CL` (single scan, includes MT5 order preview) · `--status` (recent predictions) · `--doctor` (preflight: MT5 package/connection/demo-guard/symbol resolution/price feed/killzone — auto-discovers the broker's real gold+oil symbol names when the configured ones don't exist)  
