"""
Subscriber lifecycle for ShowNotes.

The fulfill_cycle() loop in shownotes.tools reads sn_subscribers.json
and emails new shownotes to every active row. But the file was
consumed and never written — subscribers had to be hand-edited in.

Three plans matching CLAUDE.md product description:
  per_episode_29           $29 one-time per episode delivered ($0 MRR,
                           $29 one-time per terminal delivery)
  monthly_99               $99/mo for 4 episodes (recurring; diagnose
                           checks the 4/mo cap)
  monthly_unlimited_297    $297/mo unlimited (recurring; no cap)

State file: data/sn_subscribers.json — list of:
  {
    "email":         "creator@example.com",
    "name":          "Sam Creator",
    "plan":          "per_episode_29" | "monthly_99" | "monthly_unlimited_297",
...
Log: data/sn_subscription_log.json — append-only audit trail.
"""
...
SUBS_FILE = DATA_DIR / "sn_subscribers.json"
LOG_FILE  = DATA_DIR / "sn_subscription_log.json"

PLANS = {
    "per_episode_29":        {"price_mo": 0,   "one_time": 29,
                              "monthly_cap": 0,
                              "label": "Per-episode ($29 one-time per delivery)"},
    "monthly_99":            {"price_mo": 99,  "one_time": 0,
                              "monthly_cap": 4,
                              "label": "Monthly 4-episode ($99/mo)"},
    "monthly_unlimited_297": {"price_mo": 297, "one_time": 0,
                              "monthly_cap": -1,  # unlimited
...
def activate(email: str) -> dict:
    ...
    subs = _load(SUBS_FILE, [])
    if not isinstance(subs, list):
        return {"error": "sn_subscribers.json wrong shape"}
    for s in subs:
...
def fulfill(email: str) -> dict:
    """Terminal status for per_episode_29 (one-time)."""
    email = _norm_email(email)
    subs = _load(SUBS_FILE, [])
    if not isinstance(subs, list):
        return {"error": "sn_subscribers.json wrong shape"}
...
def _cli():
    import argparse
    p = argparse.ArgumentParser(description="ShowNotes subscriber lifecycle")
    ...
    p_ful = sub.add_parser("fulfill", help="Mark per_episode_29 as fulfilled")
    ...
    if args.cmd == "add":
        ...
        for s in out["subscribers"]:
            print(f"  {s.get('status','?'):>9s}  {s.get('plan',''):<22s}  "
                  f"{s.get('email','')}")
