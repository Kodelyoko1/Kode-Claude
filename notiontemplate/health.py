"""NotionTemplate health: per-template outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "nt_inputs"
OUTPUTS_DIR = DATA_DIR / "nt_outputs"
FILE_LOG    = DATA_DIR / "nt_template_log.json"
DELIVERY    = DATA_DIR / "nt_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("NT_LOG_MAX", "300"))
VALID = {'success', 'build_failed', 'unknown_preset', 'spec_invalid'}
...
def record_template(slug, outcome, detail="", **extra):
    if not slug: return
...
def recent_templates(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def template_outcome_summary():
    log = _load(FILE_LOG, [])
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "nt_inputs": n_in, "nt_outputs": n_out,
