...
from notiontemplate.health import (probe_inputs, template_outcome_summary, stuck_mail_failed)
from notiontemplate.subscribers import listing as sub_listing

DATA_DIR = Path(__file__).parent.parent / "data"
LEADS = DATA_DIR / "nt_leads.json"
...
def check_inputs():
    p = probe_inputs()
    if not p.get("ok"):
        return Check("Inputs", "P0", "fail", "nt_inputs/ empty",
                     "Drop manifest at data/nt_inputs/<slug>.json")
    age = p.get("newest_age_days")
    age_s = f" newest {age}d old" if age is not None else ""
    detail = f"nt_inputs={p['nt_inputs']}  nt_outputs={p['nt_outputs']}{age_s}"
...
def check_outcomes():
    ...
    if s["total"] == 0: return Check("Outcomes", "info", "info", "(no templates logged)")
...
def check_leads():
    ...
    if not isinstance(leads, list) or not leads:
        return Check("Lead pipeline", "info", "info", "0 leads — populate nt_leads.json")
...
def main():
    print("NotionTemplate preflight\n")
