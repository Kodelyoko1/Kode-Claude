#!/usr/bin/env python3
"""TemplateForge autonomous loop — render template mockups + briefs."""
...
def cycle():
    console.print(Panel(
        ...
        title="[bold blue]Wholesale Omniverse — TemplateForge[/bold blue]",
        border_style="blue"))
    r = run_full_cycle()
    console.print(f"  [cyan]Templates built:[/cyan] {r.get('templates_produced', 0)}")
...
def main():
    ...
    if not paywall_prompt("templateforge"):
        return
    while True:
        cycle()
        if args.interval <= 0:
