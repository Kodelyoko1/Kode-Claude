...
from proofbot.health import (probe_inputs, page_outcome_summary, stuck_mail_failed)
from proofbot.subscribers import listing as sub_listing

DATA_DIR = Path(__file__).parent.parent / "data"
LEADS = DATA_DIR / "pf_leads.json"
...
def check_inputs():
    p = probe_inputs()
    if not p.get("ok"):
        return Check("Inputs", "P0", "fail", "pf_inputs/ empty",
                     "Drop manifest at data/pf_inputs/<slug>.json")
    age = p.get("newest_age_days")
    age_s = f" newest {age}d old" if age is not None else ""
    detail = f"pf_inputs={p['pf_inputs']}  pf_outputs={p['pf_outputs']}{age_s}"
...
def check_leads():
    ...
    if not isinstance(leads, list) or not leads:
        return Check("Lead pipeline", "info", "info", "0 leads — populate pf_leads.json")
