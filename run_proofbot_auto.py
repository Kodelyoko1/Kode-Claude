#!/usr/bin/env python3
"""ProofBot autonomous loop — proofread sources, deliver reports, pitch leads."""
...
def cycle():
    console.print(Panel(
        ...
        title="[bold blue]Wholesale Omniverse — ProofBot[/bold blue]",
        border_style="blue"))
    r = run_full_cycle()
    console.print(f"  [cyan]Reports produced:[/cyan] {r.get('reports_produced', 0)}")
...
def main():
    ...
    if not paywall_prompt("proofbot"):
        return
    while True:
        cycle()
        if args.interval <= 0:
