"""ThumbForge health: per-thumb outcomes + delivery log."""
...
INPUTS_DIR  = DATA_DIR / "tf_inputs"
OUTPUTS_DIR = DATA_DIR / "tf_outputs"
FILE_LOG    = DATA_DIR / "tf_thumb_log.json"
DELIVERY    = DATA_DIR / "tf_delivery_outcomes.json"
LOG_MAX     = int(os.environ.get("TF_LOG_MAX", "300"))
VALID = {'success', 'build_failed', 'spec_invalid', 'missing_title'}
...
def record_thumb(slug, outcome, detail="", **extra):
    if not slug: return
...
def recent_thumbs(limit=50):
    log = _load(FILE_LOG, [])
    return log[-limit:][::-1] if isinstance(log, list) else []


def thumb_outcome_summary():
    log = _load(FILE_LOG, [])
...
def probe_inputs():
    ...
    return {"ok": n_in > 0, "tf_inputs": n_in, "tf_outputs": n_out,
