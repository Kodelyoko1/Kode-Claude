...
from thumbforge.health import (probe_inputs, thumb_outcome_summary, stuck_mail_failed)
from thumbforge.subscribers import listing as sub_listing

DATA_DIR = Path(__file__).parent.parent / "data"
LEADS = DATA_DIR / "tf_leads.json"
...
def check_inputs():
    p = probe_inputs()
    if not p.get("ok"):
        return Check("Inputs", "P0", "fail", "tf_inputs/ empty",
                     "Drop manifest at data/tf_inputs/<slug>.json")
    age = p.get("newest_age_days")
    age_s = f" newest {age}d old" if age is not None else ""
    detail = f"tf_inputs={p['tf_inputs']}  tf_outputs={p['tf_outputs']}{age_s}"
...
def check_outcomes():
    ...
    if s["total"] == 0: return Check("Outcomes", "info", "info", "(no thumbs logged)")
...
def check_leads():
    ...
    if not isinstance(leads, list) or not leads:
        return Check("Lead pipeline", "info", "info", "0 leads — populate tf_leads.json")
