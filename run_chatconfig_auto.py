#!/usr/bin/env python3
"""ChatConfig autonomous loop — render chatbot bundles, deliver, pitch leads."""
...
def cycle():
    console.print(Panel(
        ...
        title="[bold blue]Wholesale Omniverse — ChatConfig[/bold blue]",
        border_style="blue"))
    r = run_full_cycle()
    console.print(f"  [cyan]Bots packaged:[/cyan]  {r.get('bots_produced', 0)}")
...
def main():
    ...
    if not paywall_prompt("chatconfig"):
        return
    while True:
        cycle()
        if args.interval <= 0:
